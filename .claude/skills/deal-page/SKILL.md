...
The deal page is composed of four primary components. The page route (`src/app/deal/page.tsx`) renders the component tree; individual deal components live in `src/components/deal/`. Note: `src/components/deal-v2/` is deprecated and contains only stub redirects — all active code is in `src/components/deal/`.
...
Renders directly in the page orchestrator, not delegated to sub-components:
- Title (`deal.title`)
...
Fields: supplyAbility, sampleLeadTime, powerSource (array of strings), assemblyRequired, euResponsiblePerson, batteryInfo.
...
### PricingPanel (pricing-panel.jsx, ~865 lines)
...
- Order terms: moq, orderIncrement, casePackSize, availableQuantity, maxOrderQuantity. Note: `moqUnit` has been removed — it is now derived from `priceUnit` at display time via a `moqUnitLabel` computed variable that extracts the unit word from priceUnit, pluralizes it, and preserves any suffix (e.g. "ex. VAT").
...
### ProductDescriptionTabs (product-tabs.jsx, ~1302 lines)
...
### SupplierSidebarCard (supplier-sidebar.jsx, ~987 lines)
...
### ImageGallery (image-gallery.jsx, ~148 lines)
...
- **dealStatus:** "active", "sold-out" (only these two values are exposed in the deal form; demo-data.js includes additional values like "discontinued", "end-of-life", "pre-launch", "seasonal" for display/testing)
...
Removed enums: `priceType` (redundant with pricing tiers + negotiable flag), `sellingUnit` (consolidated into `priceUnit`), `moqUnit` (derived from `priceUnit` at display time).
...
| File | Lines | Contains |
|------|-------|----------|
| `src/components/deal/pricing-panel.jsx` | ~865 | Pricing, margin, volume tiers, order terms, product specs |
| `src/components/deal/product-tabs.jsx` | ~1302 | 5-tab content: description, specs, compliance, reviews, attachments |
| `src/components/deal/supplier-sidebar.jsx` | ~987 | Supplier card (see `supplier-profile` skill) |
| `src/components/deal/image-gallery.jsx` | ~148 | Product images, margin badge, lightbox |
| `src/components/deal/demo-data.js` | ~5427 | All ~230 variables across Groups A-N, 3 category presets, 12 compact category deals |
| `src/components/shared/form-fields.jsx` | ~2619 | Shared UI components used across deal and profile pages |
