...
### FAQ Deflection System (Contact Form)

Added 2026-03-08. The contact form now includes a client-side FAQ deflection feature that suggests relevant FAQ answers before the user submits their message.

**Architecture:**
- `src/data/faq-data.js` — FAQ entries with weighted keyword maps + `matchFAQs()` scoring function
- `src/app/api/faq-suggest/route.js` — POST endpoint that calls `matchFAQs()` server-side (for future use / analytics)
- `src/components/phases/contact.jsx` — Inline `FAQSuggestionPanel` component with accordion answers

**How matching works:**
- Debounced (450ms) on subject + message field changes
- Tokenizes combined text, scores against keyword weights per FAQ entry
- Returns top 3 matches above threshold score of 3
- Multi-word keywords match as phrases; single-word keywords match on word boundaries

**UX flow:**
1. User types in subject/message → amber "We might already have the answer" panel appears below textarea
2. User expands an FAQ → "This answered my question" button appears
3. If resolved → form clears, green confirmation with "I still need to contact support" fallback
4. If dismissed (X button) → panel hides, user continues to submit normally
5. Panel never blocks Send Message — the submit button stays fully accessible

**Production migration notes:**
- 🔧 Migrate FAQ_DATA to database table with admin CMS for content management
- 🔧 Add analytics tracking: which FAQs resolve tickets, click-through rates
- 🔧 Add rate limiting to `/api/faq-suggest` (30 req/min per IP)
- 🔧 Consider semantic search (embeddings) if keyword matching proves insufficient
- 🔧 Currently runs client-side via import; API route exists for future server-side use / analytics
