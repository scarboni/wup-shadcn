...
Premium, Premium+, and Supplier Pro accounts can click up to 500 supplier website URLs per month. The click limit applies to outbound links routed through `/go?url=...&type=website`. When the limit is reached, the link shows a "Limit reached (500/mo)" badge instead of the clickable link.

The 500-click counter resets to 0 every **30 days from the user's latest upgrade date** to an eligible tier. See the dedicated `website-click-limits` skill for full reset rules, billing cycle logic, database schema, and API route specification.
